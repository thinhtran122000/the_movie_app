...
import 'package:tmdb/ui/pages/rated/views/tv/bloc/rated_tv_bloc.dart';
import 'package:tmdb/utils/app_utils/app_utils.dart';
import 'package:tmdb/utils/utils.dart';

class RatedTvView extends StatelessWidget {
  const RatedTvView({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => RatedTvBloc()
        ..add(FetchData(
          language: 'en-US',
      ...
      child: BlocListener<TmdbBloc, TmdbState>(
        listener: (context, state) {
          if (state is TmdbFavoritesSuccess ||
            ...
            BlocProvider.of<RatedTvBloc>(context).add(FetchData(
              language: 'en-US',
              sortBy: 'created_at.desc',
            ));
          }
        },
        child: BlocConsumer<RatedTvBloc, RatedTvState>(
          listener: (context, state) {
            final bloc = BlocProvider.of<RatedTvBloc>(context);
            if (state is RatedTvSortSuccess) {
            ...
            if (state is RatedTvAddWatchlistSuccess) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    (state.listTvState[state.index].watchlist ?? false)
            ...
            return SmartRefresher(
              ...
              child: Column(
                ...
                children: [
                  ...
                  BlocBuilder<RatedTvBloc, RatedTvState>(
                    builder: (context, state) {
                      if (state is RatedTvInitial) {
                      ...
                      if (state is RatedTvSuccess && state.listRated.isEmpty) {
  ...
  Widget itemBuilder(BuildContext context, int index) {
    ...
    return QuaternaryItem(
      heroTag: '${AppConstants.ratedTvTag}-${item.id}',
      ...
      onTapItem: () => Navigator.of(context).pushNamed(
        ...
        arguments: {
          ...
          'hero_tag': '${AppConstants.ratedTvTag}-${item.id}',
        },
      ),
    );
  }
